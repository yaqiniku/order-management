...
public class ProductRepository(AppDbContext context) : BaseRepository(context), IProductRepository
{
    public async Task<List<Product>> GetRows(IDbTransaction transaction, string? keyword, int offset, int limit)
    {
        ...
        var query = $@"
            select
                    ...
                    p.cre_date as CreDate,
                    p.mod_date as ModDate
            from
                    product p
            where
                    lower(coalesce(p.id, '')) like lower({p}Keyword)
    ...
    {
        var p = db.Symbol();
        var query = $"delete from product where id = {p}ID";
        return _command.Delete(transaction, query, new { ID = id });
    }

    public Task<Product?> GetProductStock(IDbTransaction transaction, string id)
    {
        var p = db.Symbol();
        var query = $@"
            select
    ...
    public Task<int> UpdateQty(IDbTransaction transaction, Product product)
    {
        var p = db.Symbol();
